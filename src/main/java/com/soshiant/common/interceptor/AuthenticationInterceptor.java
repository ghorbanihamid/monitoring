...
public class AuthenticationInterceptor implements Interceptor {
    ...
    public String intercept(ActionInvocation actionInvocation) throws Exception {
        ...
        UserInfo user = (UserInfo) session.get(ServerConstants.OBJECT_USERNAME);
    ...
    private void purgeStaleTokens(Map session) {
        ...
        /* Due to the structure of our sample app, we might have a UserInfo token of a
           * different type stored, i.e. a UserInfo set into the session scope by one of
           * the other versions of the app, a different chapter package's UserInfo.
           *
           * If the UserInfo token is not of the right type, we'll just throw it away.
           * A sort of forced logout when switching between versions of the application.
           */

        Object userToken = session.get(ServerConstants.OBJECT_USERNAME);
        if (!(userToken instanceof UserInfo)) session.remove(ServerConstants.OBJECT_USERNAME);
